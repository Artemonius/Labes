...
// === использование для всех методов ===
// минимальный размер 2
vector<int> CreateVector(int size = 20)
{
	if (size < 2) size = 20;
	srand(time(0));
	vector<int> res;
...
// серии должны быть отсортированы
...
// === естественная часть сортировки слиянием ===
...
vector<vector<int>> SplitToBalancedSeries(vector<int> vec)
{
	...
	while (isVectorNeedToSplit)
	{
		...
		for (int i = 0; i < series.size(); i++)
		{
			// деление серии, если ее размерность больше двух
			if (series[i].size() > 2)
			{
				...
				// разное условие для нечетного и нечетного числа элементов в серии
			...
			// сортировка двух элементов в серии
...
vector<int> BalancedMergeSort(vector<int> vec)
{
	...
	// сортировка нескольких элементов
	vector<vector<int>> newSeries;
...
// === Многофазная часть сортировки слиянием ===
...
// функция Фибоначчи для поиска размерности рядов
...
{
	// поиск доступного количества элементов в серии
...
vector<int> PolyphaseMergeSort(vector<int> vec)
{
	...
	vector<vector<int>> F1, F2, F3; // файлы

	cout << "Деление на равные ряды" << endl;
	PrintSeries(series);
	...
	// Сортировка элементов внутри серий
	cout << "Сортировка элементов в файлах" << endl;
...
int main()
{
	...
	cout << "Источник вектора" << endl;
	vector<int> vec = CreateVector();
	PrintVector(vec);
	cout << endl << "-----Естественная сортировка слиянием-----" << endl;
	vector<int> vec1 = NaturalMergeSort(vec);
	PrintVector(vec1);
	cout << endl << "-----Сбалансированная сортировка-----" << endl;
